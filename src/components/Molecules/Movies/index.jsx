            ...
            movies?.map((movie) => (
              <div key={movie.id} className="relative rounded-lg">
                ...
                <div className="absolute backdrop-blur-sm bg-[#e50914] bg-opacity-50 p-2 opacity-0 hover:opacity-100 transition ease-in-out duration-150 h-full w-full bottom-0 rounded-lg">
                  <div className="text-sm md:text-base font-bold line-clamp-2 pb-2">
                    {movie?.title}
                  </div>
                  <div className="flex items-center text-xs md:text-sm pb-2 mb-2 border-b">
                    <AiFillStar /> {movie?.vote_average.toFixed(1)}
                  </div>
                  <div className="text-xs md:text-sm mb-2">
                    {movie.release_date}
                  </div>
                  <p className="text-xs md:text-sm line-clamp-2 sm:line-clamp-3 lg:line-clamp-6">
                    {movie?.overview}
                  </p>
                </div>
